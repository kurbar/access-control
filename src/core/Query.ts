...
class Query {
  ...
  on(
    ...
    return this._getPermission(
      this._.action,
      resource,
      skipConditions || this._.skipConditions,
      this._.checkInSync,
    );
  }
   ...
   *         Overwrite the existing context, merge if false (default)
   *  @returns {Query}
   *           Self instance of `Query`.
   */
  context(context: any, overwrite: boolean = false): Query {
    if (overwrite) {
      this._.context = context;
    } else {
      this._.context = { ...this._.context, ...context };
    }
    return this;
  }
